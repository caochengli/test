...
<link rel="stylesheet" type="text/css" href="/Public/admin/plugins/zTreeStyle/zTreeStyle.css" />
<link rel="stylesheet" href="/Public/admin/dist/css/jquery-ui-1.10.3.full.min.css" />
        ...
        <div class="col-xs-12">
          <div class="box box-danger">
            <div class="box-header with-border">                
                <a class="btn btn-success btn-sm btn-sub-taxis" href="javascript:void(0);"><i class="fa fa-sort-alpha-desc"></i> 更新排序</a> <a class="btn btn-info btn-sm" href="<?php echo U('/Role/addusergroup');?>"><i class="fa fa-plus"></i> 添加用户组</a>
             </div>
            <!-- /.box-header -->
            <div class="box-body">
             ...
             <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                   <?php if(is_array($usergrouparr)): $i = 0; $__LIST__ = $usergrouparr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rs): $mod = ($i % 2 );++$i;?><tr>
                      <td class="text-center">
                         <input class="input-mini text-center" size="4" value="<?php echo ($rs["taxis"]); ?>" data-id="<?php echo ($rs["id"]); ?>" type="text" name="fortaxis">
                      </td>
                      <td><?php echo ($rs["name"]); ?></td>
                      <td class="text-muted"><?php echo ($rs["desc"]); ?></td>
                      <td class="text-center">
                         
                            <a class="btn btn-xs btn-info set-pagelevel" href="javascript:void(0);" data-group-id="<?php echo ($rs["id"]); ?>">
                               <i class="fa fa-edit"></i> 页面权限
                            </a>
                            <a class="btn btn-xs btn-primary set-catearr" href="javascript:void(0);" data-group-id="<?php echo ($rs["id"]); ?>">
                      ...
                      <td class="text-center">
                            ...
                            <a class="btn btn-xs btn-info" href="<?php echo U('/Role/editusergroup',array('id'=>$rs['id']));?>">
...
<script src="/Public/admin/plugins/jQueryUI/jquery-ui-1.10.3.full.min.js"></script>
<script language="javascript" src="/Public/admin/plugins/zTreeStyle/jquery.ztree.core-3.5.min.js"></script>
<script language="javascript" src="/Public/admin/plugins/zTreeStyle/jquery.ztree.excheck-3.5.min.js"></script>
<script language="javascript">
var setting={check:{enable:true},data:{simpleData:{enable:true}}};
function del(id){
   var url = "<?php echo U('/Role/delete/id/"+id+"/tabel/admin_group/back/usergroup');?>";
   //url += '/id/'+id;
   $.ajax({
	 url : url,
	 type : "GET", 
...
$(function(){
   $('.btn-sub-taxis').click(function(){
	  ...
	  $.ajax({
			 url: "<?php echo U('/Role/updatetaxis',array('tabel'=>'admin_group','back'=>'usergroup'));?>", 
			 type: "POST",
   ...
   $('.set-pagelevel').click(function(){
	  ...
	  $.ajax({
	     url : "<?php echo U('/Role/getpowerjson/groupid/"+g+"');?>",
		 type : 'POST',
		 success: function(d){
		    if(!!d){
			   zNodes = $.parseJSON(d);
   ...
   $('.set-catearr').click(function(){
	  ...
	  var g = $(this).attr('data-group-id');
	  $.ajax({
	     url : "<?php echo U('/Role/getcatejson/groupid/"+g+"');?>",
		 type : 'POST',
		 success: function(d){
		    if(!!d){
			   zNodes = $.parseJSON(d);
	  ...
	  buttons: [
			  ...
			  },
			  {
				  ...
				  click: function() {
				     //$( this ).dialog( "close" ); 
					 if(title == '页面权限编辑'){					    
						savepower($(this),"<?php echo U('/Role/savepagelevel/groupid/"+gid+"');?>");
					 }else{
						savecatepower($(this),"<?php echo U('/Role/savecatelevel/groupid/"+gid+"');?>");
					 }
				  } 
			  }
	  ]  
